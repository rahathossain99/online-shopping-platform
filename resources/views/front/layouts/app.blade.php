            ...
            <div class="col-lg-6 col-6 text-left  d-flex justify-content-end align-items-center">
                ...
                <form action="{{ route('front.shop') }}" method="get">
                    <div class="input-group">
                        <input type="text" placeholder="Search For Products" class="form-control" value="{{ Request::get('search') }}" id="search" name="search">
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="wishlistModelTitle">Success</h5>
            ...
            success: function (response) {
                if (response.status == true) {
                    if(response.exist==true){
                        ...
                        $('#wishlistModel .modal-body').html(message);
                        $('#wishlistModelTitle').html('Failed');
                    }else{
                        var message=`<div class="alert alert-success">${response.message}</div>`;
                        $('#wishlistModel').modal('show');
                        $('#wishlistModel').modal('show');
                        $('#wishlistModel .modal-body').html(message);
                        $('#wishlistModelTitle').html('Success');
                    }
                } else {
