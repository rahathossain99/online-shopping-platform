                            ...
                            <tbody>
                                @foreach($cartContents as $cartContent)
                                     <tr>
                                        ...
                                        <td>
                                            <button onclick="deleteCart('{{ $cartContent->rowId }}')" class="btn btn-sm btn-danger"><i class="fa fa-times"></i></button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
    ...
    <script>
        ...
        $('.sub').click(function(){
            ...
            if (qtyValue > 1) {
                ...
                qtyElement.val(qtyValue-1);
                var newQty=qtyElement.val();
                updateToCart(rowId,newQty,'sub');
            }
        });

        function updateToCart(rowId,newQty,cal)
        {
            $.ajax({
                url:'{{ route('front.updateToCart') }}',
                type:'post',
                data:{rowId:rowId,qty:newQty,cal:cal},
                dataType:'json',
                headers: {
